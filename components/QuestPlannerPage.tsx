...
export const QuestPlannerPage: React.FC<QuestPlannerPageProps> = ({ onBack }) => {
   ...
   const [newTaskData, setNewTaskData] = useState<{ title: string, desc: string, points: number, diff: QuestDifficulty, assignedToId: string | null }>({
      title: '', desc: '', points: 50, diff: QuestDifficulty.EASY, assignedToId: null
   });
   ...
   const handleCreateTask = () => {
      ...
      setNewTaskData({ title: '', desc: '', points: 50, diff: QuestDifficulty.EASY, assignedToId: null });
            ...
            {view === 'tasks' && (
               <div className="space-y-6 animate-in fade-in duration-500">
                  ...
                  <div className="grid gap-4">
                     {tasks.filter(t => {
                        if (t.status === QuestStatus.DONE) return false;
                        if (isAdmin) return true;
                        return t.assignedToId === null || t.assignedToId === currentUser.id;
                     }).map(task => (
            ...
            <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
               <div className="bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl">
                  ...
                  <div className="space-y-4">
                     ...
                     <div>
                        <label className="block text-xs font-bold uppercase text-gray-400 mb-1">Asignar a (Opcional)</label>
                        <select
                           value={(editingTask ? editingTask.assignedToId : newTaskData.assignedToId) || ''}
                           onChange={e => {
                              const val = e.target.value || null;
                              if (editingTask) setEditingTask({ ...editingTask, assignedToId: val });
                              else setNewTaskData({ ...newTaskData, assignedToId: val as any });
                           }}
                           className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-black"
                        >
                           <option value="">Cualquiera (Tablón Público)</option>
                           {users.filter(u => u.role === UserRole.CHILD).map(u => (
